...
from book_library_app.utils import get_schema_args, apply_order, apply_filter, get_pagination, \
    validate_json_content_type, token_required
...
def get_authors():
    ...
    authors = BookSchema(**schema_args).dump(items)
...
def create_book(user_id: int, args, author_id: int):
    Author.query.get_or_404(author_id, description=f'Author with id {author_id} not found')
    if Book.query.filter(Book.isbn == args['isbn']).first():
        abort(409, description=f'Book with ISBN {args["isbn"]} already exists')
