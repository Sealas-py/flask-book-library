...
def register(args: dict):
    if User.query.filter(User.username == args['username']).first():
        abort(code=409, description=f'User with username {args["username"]} already exists')
    if User.query.filter(User.email == args['email']).first():
        abort(code=409, description=f'User with email {args["email"]} already exists')
...
def login(args: dict):
    ...
    if not user:
        abort(code=401, description=f'Invalid credentials')
